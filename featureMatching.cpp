...
int main() {
    ...
    std::string target_image_path = "C:/Users/Shi Zhang/My Drive/CS/NEU Align/Courses/2023 Fall/5330/Project02/olympus/pic.1016.jpg";
    int N = 3;

    cv::Mat target_img = cv::imread(target_image_path, cv::IMREAD_COLOR);
    if (target_img.empty()) {
        std::cerr << "Could not read the target image: " << target_image_path << std::endl;
        return -1;
    }

    int center_x = target_img.cols / 2;
    int center_y = target_img.rows / 2;
    cv::Rect roi(center_x - 4, center_y - 4, 9, 9);
    cv::Mat feature_mat = target_img(roi).clone();
    ...
    */
    ...
    for (size_t i = 0; i < data.size(); i++) {
        ...
        if (target_image_path != compare_filename) {
    ...
    std::cout << "Top " << N << " Matches for " << target_image_path << " are: " << std::endl;
